...
test('run-result 信封同时记录成功和失败，成功项不携带 payload', (t) => {
  ...
  fs.writeFileSync(path.join(videosDir, '7662298104951033095', 'meta.json'), JSON.stringify({ status: 'new', analysis_error: 'timeout', cover_error: 'HTTP 404' }));
  ...
  assert.equal(items[1].errorSummary, 'timeout');
});
